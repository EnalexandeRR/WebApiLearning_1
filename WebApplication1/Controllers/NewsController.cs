...
public class NewsController : ControllerBase
{
    ...
    public async Task<IActionResult> DeleteNewsByPeriod([FromBody] DeleteByPeriodRequest request)
    {
        var deletedNewsCount = await _newsService.DeleteNewsByPeriodAsync(request);
        if (deletedNewsCount > 0)
            return Ok(new BaseResponse { StatusCode = 0, Message = $"{deletedNewsCount} news deleted!" });
        
        return Ok(new BaseResponse { StatusCode = 0, Message = "No news found for given period!" });
    }
}